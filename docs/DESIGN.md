...
6. **100일차 알이 보유자 손에 없을 때**: "보유자 없음" 방송 후 우승을 연기하고,
   그 뒤 **알을 얻는 첫 번째 플레이어가 즉시 우승**.
7. **안개 끄기 금지**: **구현하지 않음**(클라/리소스팩 별도 전제).
8. **금지 아이템**: **소지·제작·사용·획득 전면 금지**(엘리트라/셜커/엔더상자/방패/
   토템/삼지창). 단 **엔더진주는 원 스펙대로 "투척만" 금지**(소지 허용).
9. **금지 인챈트**: 신규 획득 차단 + **기존 부여분도 감지 시 삭제**.
10. **네더라이트 형판**: `다이아 7 + 네더랙 + 네더라이트 주괴 1` 레시피, 결과 1개,
    **바닐라 복제 비활성화**.
11. **최대레벨 강조**: 이름 빨강 + 로어 강조 **둘 다 유지**.
12. **황금사과**: 일반 + **노치(인챈트) 사과 모두** 재생 지속시간 감소.
13. **게임 시작**: 자동 시작 폐지. **`/DE start [N]`** 로 시작하며 지정 시 N일차부터.

### 확정됨 (3차)

14. **사망 제단**: 3×3 철블록 베이스 위에 신호기를 조약돌/이끼조약돌 계단으로
    둘러싸고(높은 면이 신호기를 향함), 보라 유리로 빔을 착색한 뒤 **신호기 위 중앙을
    y=319까지 방벽으로 채워** 빔 경로를 확보·보호한다. 알은 모서리 받침대 위에 안치.
15. **발자국 영속화**: 발자국을 실제 디스플레이 엔티티로 저장하고 생성 시각을 PDC에
    기록, 재시작 시 재채택하여 수명을 이어간다(로드된 청크 한정).
16. **제단 영속화 + 카운트다운**: 제단(블록 스냅샷 포함)을 `altars.yml`에 저장해
    100분 철거 타이머가 재시작에도 유지된다. 제단 위에 텍스트 디스플레이로 남은
    시간("N분 M초 후 소멸")을 매초 실시간 표시한다.
17. **알 보관 차단**: 드래곤 알은 상자/통/호퍼/발사기/분배기/화로류/양조대 등
    컨테이너에 넣을 수 없다(클릭·드래그·핫바 스왑·호퍼 흡입·자동화 이동 차단).
18. **관리자 명령**: `/DE status`(진행 상태·날짜·보유자 조회), `/DE pause`(일시정지/
    재개 토글 — 날짜 카운트 동결).
19. **우승 세리머니**: 우승 확정 시 모든 플레이어를 오버월드 스폰으로 이동시키고,
    스폰 위에 무적 엔더 드래곤을 소환해 승자를 태운다(승자 이름 표시). 드래곤은
    AI를 끄고 매 틱 **승자가 바라보는 방향으로 이동**하도록 조종된다.

### 알려진 한계 / 추후 확인

- 방벽이 신호기 빔을 통과시키는지, 계단 `facing` 방향(높은 면)이 의도대로인지는
  **인게임 실측 확인** 필요(안 되면 방벽→유리, facing 반전으로 조정).
- 발자국 재채택은 **언로드된 청크**의 것은 즉시 추적 못 함(만료가 지연될 수 있음).
- 제단 철거는 in-memory 타이머라 100분 내 재시작 시 방벽 기둥이 남을 수 있음.
