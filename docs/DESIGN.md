...
1. **Phase 0 — 리팩터링**: 기존 단일 리스너를 §2 패키지 구조로 재배치, `PluginConfig`
   도입. (동작 변화 없음)
2. **Phase 1 — 드래곤 알 코어**: `EggManager`(상태추적+영속화) → 차원 이동 금지 →
   사망 제단+신호기 → 발자국 → 신호기 소리/거래 제한/엔더 이펙트 → 승리 판정.
3. **Phase 2 — 전역/환경 룰**: 월드보더, 게임룰(좌표/채팅/발전과제), 폭발 데미지,
   경험치 드롭, 황금사과, 포션 제한.
4. **Phase 3 — 아이템/인챈트 마무리**: 금지 아이템/인챈트, 네더라이트 형판,
   재소환 XP, 최대레벨 장비 이름 빨강.

각 Phase는 독립 커밋으로 진행하고, Paper 1.21.8에 대해 빌드 검증 후 푸시한다.

---

## 9. 확정이 필요한 결정 목록 (요약)

1. **"1일" 정의 & 승리 자동화**: MC 낮/밤 1주기(24000틱)=1일, 플러그인 자동 판정 맞나?
2. **100일차 순간 알이 보유자 손에 없을 때**의 우승 처리.
3. **경험치 드롭 / 재소환 XP**의 스코프(전역 vs 보유자 한정).
4. **엔더맨 이펙트**의 구체 정의.
5. **발자국 표현 방식**(파티클 vs 디스플레이) & **제단 구조물 형태**.
6. **안개/좌표 강제** 수준(리소스팩·모드 병행 전제 여부).
7. **채팅 금지 범위**(전면 vs 정보성만).
8. **금지 아이템** 처리 수준(소지 불가 vs 사용만 불가).
9. **금지 인챈트/네더라이트 형판** 세부(기존 부여분 무효화 여부, 레시피 구성).
10. **최대레벨 강조**: 로어 유지 여부 + 이름 빨강 추가.
